...
test('test find-node-directory - node install', (t) => {
  ...
  for (const platform in platforms) {
    const processObj = { execPath: '/x/y/bin/node', platform }
    t.equal(
...
test('test find-node-directory - node in build release dir', (t) => {
  ...
  for (const platform in platforms) {
    let processObj
    if (platform === 'win32') {
      processObj = { execPath: '/x/y/Release/node', platform }
    } else {
      processObj = { execPath: '/x/y/out/Release/node', platform }
    }
...
test('test find-node-directory - node in Debug release dir', (t) => {
  ...
  for (const platform in platforms) {
    let processObj
    if (platform === 'win32') {
      processObj = { execPath: '/a/b/Debug/node', platform }
    } else {
      processObj = { execPath: '/a/b/out/Debug/node', platform }
    }
...
test('test find-node-directory - not found', (t) => {
  t.plan(platforms.length)
  for (const platform in platforms) {
    const processObj = { execPath: '/x/y/z/y', platform }
