;;; nnabt.el - font-lock-mode support for nnabt files.
...
(defadvice python-indent-calculate-levels (after nnabt-outdent-closing-parens
                                                 activate)
  "De-indent closing parens, braces, and brackets in nnabt-mode."
  (when (and (eq major-mode 'nnabt-mode)
             (string-match "^ *[])}][],)}]* *$"
...
(defadvice python-indent-guess-indent-offset (around
                                              nnabt-indent-guess-indent-offset
                                              activate)
  "Guess correct indent offset in nnabt-mode."
  (or (and (not (eq major-mode 'nnabt-mode))
           ad-do-it)
      (save-excursion
        (save-restriction
          (widen)
          ...
          (or (and (re-search-forward "\\(^[[{]$\\|^.*[^#].*[[{]$\\)")
                   ...
                   (set (make-local-variable 'python-continuation-offset)
                        (current-indentation)))
              (message "Can't guess nnabt indent offset, using default: %s"
                       python-continuation-offset))))))

(define-derived-mode nnabt-mode python-mode "Gyp"
  "Major mode for editing .nnabt files. See http://code.google.com/p/nnabt/"
  ;; nnabt-parse-history is a stack of (POSITION . PARSE-STATE) tuples,
  ;; with greater positions at the top of the stack. PARSE-STATE
  ;; is a list of section symbols (see nnabt-section-name and nnabt-parse-to)
  ;; with most nested section symbol at the front of the list.
  (set (make-local-variable 'nnabt-parse-history) '((1 . (list))))
  (nnabt-add-font-lock-keywords))

(defun nnabt-set-indentation ()
  "Hook function to configure python indentation to suit nnabt mode."
...
(add-hook 'nnabt-mode-hook 'nnabt-set-indentation)

(add-to-list 'auto-mode-alist '("\\.nnabt\\'" . nnabt-mode))
(add-to-list 'auto-mode-alist '("\\.nnabti\\'" . nnabt-mode))
(add-to-list 'auto-mode-alist '("/\\.gclient\\'" . nnabt-mode))

;;; Font-lock support

(defconst nnabt-dependencies-regexp
  (regexp-opt (list "dependencies" "export_dependent_settings"))
  "Regular expression to introduce 'dependencies' section")

(defconst nnabt-sources-regexp
...
(defun nnabt-section-name (section)
  ...
  (cond ((string-match-p nnabt-dependencies-regexp section) 'dependencies)
        ((string-match-p nnabt-sources-regexp section) 'sources)
        ((string-match-p nnabt-variables-regexp section) 'variables)
        ((string-match-p nnabt-conditions-regexp section) 'conditions)
        ((string-match-p nnabt-targets-regexp section) 'targets)
        ((string-match-p nnabt-defines-regexp section) 'defines)
        (t 'other)))

(defun nnabt-invalidate-parse-states-after (target-point)
  "Erase any parse information after target-point."
  (while (> (caar nnabt-parse-history) target-point)
    (setq nnabt-parse-history (cdr nnabt-parse-history))))

(defun nnabt-parse-point ()
  "The point of the last parse state added by nnabt-parse-to."
  (caar nnabt-parse-history))

(defun nnabt-parse-sections ()
  "A list of section symbols holding at the last parse state point."
  (cdar nnabt-parse-history))

(defun nnabt-inside-dictionary-p ()
  "Predicate returning true if the parser is inside a dictionary."
  (not (eq (cadar nnabt-parse-history) 'list)))

(defun nnabt-add-parse-history (point sections)
  "Add parse state SECTIONS to the parse history at POINT so that parsing can be
   resumed instantly."
  (while (>= (caar nnabt-parse-history) point)
    (setq nnabt-parse-history (cdr nnabt-parse-history)))
  (setq nnabt-parse-history (cons (cons point sections) nnabt-parse-history)))

(defun nnabt-parse-to (target-point)
  "Parses from (point) to TARGET-POINT adding the parse state information to
   nnabt-parse-state-history. Parsing stops if TARGET-POINT is reached or if a
  ...
  (let ((parsing t)
    ...
    (while parsing
      ...
      ;; level decreases.
      (let ((state (parse-partial-sexp (nnabt-parse-point)
                                       target-point
                                       -1
                                       t))
            (sections (nnabt-parse-sections)))
        (if (= (nth 0 state) -1)
            (setq sections (cdr sections)) ; pop out a level
          (cond ((looking-at-p "['\"]") ; a string
                 (setq string-start (point))
                 (goto-char (scan-sexps (point) 1))
                 (if (nnabt-inside-dictionary-p)
                     ;; Look for sections inside a dictionary
                     (let ((section (nnabt-section-name
                                     (buffer-substring-no-properties
                                      (+ 1 string-start)
                                      (- (point) 1)))))
                ...
                ((not (eobp))
                 ;; other
                 (forward-char 1))))
        (nnabt-add-parse-history (point) sections)
        (setq parsing (< (point) target-point))))
    string-start))

(defun nnabt-section-at-point ()
  "Transform the last parse state, which is a list of nested sections and return
   ...
   section."
  (let ((sections (nnabt-parse-sections)))
    (cond
...
(defun nnabt-section-match (limit)
  "Parse from (point) to LIMIT returning by means of match data what was
   ...
   See also `nnabt-add-font-lock-keywords'."
  (nnabt-invalidate-parse-states-after (point))
  (let ((group nil)
        (string-start t))
    (while (and (< (point) limit)
                (not group)
                string-start)
      (setq string-start (nnabt-parse-to limit))
      (if string-start
          (setq group (case (nnabt-section-at-point)
                        ('dependencies 1)
                        ('variables 2)
                        ('conditions 2)
                        ('sources 3)
...
;;; Please see http://code.google.com/p/nnabt/wiki/GypLanguageSpecification for
;;; canonical list of keywords.
(defun nnabt-add-font-lock-keywords ()
  "Add nnabt-mode keywords to font-lock mechanism."
  ;; TODO(jknotten): Move all the keyword highlighting into nnabt-section-match
