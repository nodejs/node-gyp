...
import nnabt.MSVSUtil
import nnabt.MSVSVersion
...
def GetGlobalVSMacroEnv(vs_version):
    """Get a dict of variables mapping internal VS macro names to their nnabt
...
class MsvsSettings(object):
    """A class that understands the nnabt 'msvs_...' values (especially the
  ...
  class helps map those settings to command line options."""
    ...
    """
        ...
        if ext:
            return ext
        return nnabt.MSVSUtil.TARGET_TYPE_EXT.get(self.spec["type"], "")
    ...
    def _TargetConfig(self, config):
        ...
        # "the" config at the nnabt level, which will be something like 'Debug' or
    ...
    def _GetAdditionalLibraryDirectories(self, root, config, nnabt_to_build_path):
    ...
    setting."""
        ...
        libpaths = [
            os.path.normpath(nnabt_to_build_path(self.ConvertVSMacros(p, config=config)))
            for p in libpaths
        ]
        return ['/LIBPATH:"' + p + '"' for p in libpaths]

    def GetLibFlags(self, config, nnabt_to_build_path):
        ...
        libflags.extend(
            self._GetAdditionalLibraryDirectories(
                "VCLibrarianTool", config, nnabt_to_build_path
            )
        )
    ...
    def GetDefFile(self, nnabt_to_build_path):
        ...
        if spec["type"] in ("shared_library", "loadable_module", "executable"):
            ...
            if len(def_files) == 1:
                return nnabt_to_build_path(def_files[0])
            elif len(def_files) > 1:
                raise Exception("Multiple .def files")
        return None

    def _GetDefFileAsLdflags(self, ldflags, nnabt_to_build_path):
        """.def files get implicitly converted to a ModuleDefinitionFile for the
    linker in the VS generator. Emulate that behaviour here."""
        def_file = self.GetDefFile(nnabt_to_build_path)
        if def_file:
    ...
    manifest files."""
        ...
        self._GetDefFileAsLdflags(ldflags, nnabt_to_build_path)
        ...
        ldflags.extend(
            self._GetAdditionalLibraryDirectories(
                "VCLinkerTool", config, nnabt_to_build_path
            )
        )
    ...
    def _GetLdManifestFlags(
        self, config, name, nnabt_to_build_path, allow_isolation, build_dir
    ):
        """Returns a 3-tuple:
  ...
  <security>
    <requestedPrivileges>
      <requestedExecutionLevel level='%s' uiAccess='%s' />
        ...
        nnabt.common.EnsureDirExists(build_dir_generated_name)
        f = nnabt.common.WriteOnDiff(build_dir_generated_name)
        ...
        manifest_files += self._GetAdditionalManifestFiles(config, nnabt_to_build_path)
        return flags, output_name, manifest_files

    def _GetAdditionalManifestFiles(self, config, nnabt_to_build_path):
    ...
    generated by the linker."""
        ...
        if isinstance(files, str):
            files = files.split(";")
        return [
            os.path.normpath(nnabt_to_build_path(self.ConvertVSMacros(f, config=config)))
            for f in files
        ]
    ...
    def GetRcflags(self, config, nnabt_to_ninja_path):
    ...
    compiler."""
        ...
        rc("AdditionalIncludeDirectories", map=nnabt_to_ninja_path, prefix="/I")
        rcflags.append("/I" + nnabt_to_ninja_path("."))
...
class PrecompiledHeader(object):
  ...
  """
    ...
    def __init__(
        self, settings, config, nnabt_to_build_path, nnabt_to_unique_output, obj_ext
    ):
        self.settings = settings
        self.config = config
        pch_source = self.settings.msvs_precompiled_source[self.config]
        self.pch_source = nnabt_to_build_path(pch_source)
        filename, _ = os.path.splitext(pch_source)
        self.output_obj = nnabt_to_unique_output(filename + obj_ext).lower()
...
def GetVSVersion(generator_flags):
    global vs_version
    if not vs_version:
        vs_version = nnabt.MSVSVersion.SelectVisualStudioVersion(
    ...
    for line in output_of_set.splitlines():
        for envvar in envvars_to_save:
            if re.match(envvar + "=", line.lower()):
                ...
                if envvar == "path":
                    # Our own rules (for running nnabt-win-tool) and other actions in
...
def GenerateEnvironmentFiles(
  ...
  "-G ninja_use_custom_environment_files" to the nnabt to suppress file
  generation and use custom environment files prepared by yourself."""
    ...
    for arch in archs:
        ...
        # Inject system includes from nnabt files into INCLUDE.
        if system_includes:
...
def VerifyMissingSources(sources, build_dir, generator_flags, nnabt_to_ninja):
  ...
  so they're not surprised when the VS build fails."""
    if int(generator_flags.get("msvs_error_on_missing_sources", 0)):
        ...
        relative = [os.path.join(build_dir, nnabt_to_ninja(s)) for s in no_specials]
...
def CalculateCommonVariables(default_variables, params):
    ...
    msvs_version = nnabt.msvs_emulation.GetVSVersion(generator_flags)
