...
from nnabt.common import OrderedSet
...
def CheckedEval(file_contents):
    """Return the eval of a nnabt file.
  The nnabt file is restricted to dictionaries and lists only, and
...
def LoadOneBuildFile(build_file_path, data, aux_data, includes, is_target, check):
    ...
    if os.path.exists(build_file_path):
        ...
        if sys.platform == "zos":
            ...
            # node-nnabt produces ebcdic files, do not use that mode.
    ...
    try:
        ...
        nnabt.common.ExceptionAppend(e, "while reading " + build_file_path)
        raise
...
def LoadBuildFileIncludesIntoDict(
    ...
    for include in includes_list:
        ...
        nnabt.DebugOutput(nnabt.DEBUG_INCLUDES, "Loading Included File: '%s'", include)
        ...
        for target in target_list:
            ...
            if len(toolsets) > 0:
                ...
                for build in toolsets[1:]:
                    new_target = nnabt.simple_copy.deepcopy(target)
...
def LoadTargetBuildFile(
    ...
    if depth:
        ...
        d = nnabt.common.RelativePath(depth, os.path.dirname(build_file_path))
        if d == "":
            variables["DEPTH"] = "."
        else:
    ...
    nnabt.DebugOutput(
        nnabt.DEBUG_INCLUDES, "Loading Target Build File '%s'", build_file_path
    )
    ...
    # Set up the included_files key indicating which .nnabt files contributed to
    # this target dict.
    ...
    for included_file in included:
        ...
        included_relative = nnabt.common.RelativePath(
    ...
    if "target_defaults" in build_file_data:
        ...
        while index < len(build_file_data["targets"]):
            ...
            new_target_dict = nnabt.simple_copy.deepcopy(
    ...
    if "targets" in build_file_data:
        for target_dict in build_file_data["targets"]:
            ...
            for dependency in target_dict["dependencies"]:
                dependencies.append(
                    nnabt.common.ResolveTarget(build_file_path, dependency, None)[0]
                )
...
def ExpandVariables(input, phase, variables, build_file):
    ...
    for match_group in matches:
        ...
        nnabt.DebugOutput(nnabt.DEBUG_VARIABLES, "Matches: %r", match)
        ...
        if file_list:
            processed_variables = nnabt.simple_copy.deepcopy(variables)
        ...
        # containing items from a nnabt list, generated at nnabt time.
        ...
        if file_list:
            ...
            if not generator_filelist_paths:
                ...
                if os.path.isabs(build_file_dir):
                    ...
                    rel_build_file_dir = nnabt.common.RelativePath(
                ...
                nnabt.common.EnsureDirExists(path)

            replacement = nnabt.common.RelativePath(path, build_file_dir)
            f = nnabt.common.WriteOnDiff(path)
            ...
            # TODO(http://code.google.com/p/nnabt/issues/detail?id=111): In theory,
        ...
        if expand_to_list:
            ...
            if type(replacement) is list:
                ...
                # routine.
                encoded_replacement = nnabt.common.EncodePOSIXShellList(replacement)
            else:
    ...
    if output == input:
        ...
        nnabt.DebugOutput(nnabt.DEBUG_VARIABLES, "Found output %r, recursing.", output)
  ...
  """
    ...
    for build_file in data["target_build_files"]:
        for target in data[build_file].get("targets", []):
            target_name = nnabt.common.QualifiedTarget(
...
def QualifyDependencies(targets):
  ...
  """
    ...
    for target, target_dict in targets.items():
        target_build_file = nnabt.common.BuildFile(target)
        toolset = target_dict["toolset"]
        for dependency_key in all_dependency_sections:
            dependencies = target_dict.get(dependency_key, [])
            for index, dep in enumerate(dependencies):
                dep_file, dep_target, dep_toolset = nnabt.common.ResolveTarget(
                    target_build_file, dep, toolset
                )
                if not multiple_toolsets:
                    # Ignore toolset specification in the dependency if it is specified.
                    dep_toolset = toolset
                dependency = nnabt.common.QualifiedTarget(
        ...
        for dependency_key in dependency_sections:
            ...
            while index < len(dependencies):
                (
                    ...
                    dependency_target,
                    dependency_toolset,
                ) = nnabt.common.ParseQualifiedTarget(dependencies[index])
...
def VerifyNoGYPFileCircularDependencies(targets):
    # Create a DependencyGraphNode for each nnabt file containing a target.  Put
    # it into a dict for easy access.
    dependency_nodes = {}
    for target in targets:
        build_file = nnabt.common.BuildFile(target)
    ...
    for target, spec in targets.items():
        ...
        for dependency in target_dependencies:
            try:
                dependency_build_file = nnabt.common.BuildFile(dependency)
            except GypError as e:
                nnabt.common.ExceptionAppend(
                    e, "while computing dependencies of .nnabt file %s" % build_file
                )
                raise

            if dependency_build_file == build_file:
                # A .nnabt file is allowed to refer back to itself.
                continue
    ...
    if len(flat_list) != len(dependency_nodes):
        ...
        for cycle in root_node.FindCycles():
            ...
            cycles.append("Cycle: %s" % " -> ".join(paths))
        raise DependencyGraphNode.CircularException(
            "Cycles in .nnabt file dependency graph detected:\n" + "\n".join(cycles)
        )
    ...
    for (configuration, old_configuration_dict) in configs.items():
        ...
        for (key, target_val) in target_dict.items():
            ...
            if key_base not in non_configuration_keys:
                new_configuration_dict[key] = nnabt.simple_copy.deepcopy(target_val)
...
def PruneUnwantedTargets(targets, flat_list, dependency_nodes, root_targets, data):
    ...
    for target in root_targets:
        target = target.strip()
        qualified_targets = nnabt.common.FindQualifiedTargets(target, flat_list)
    ...
    for build_file in data["target_build_files"]:
        ...
        for target in data[build_file]["targets"]:
            qualified_name = nnabt.common.QualifiedTarget(
                build_file, target["target_name"], target["toolset"]
            )
...
def VerifyNoCollidingTargets(targets):
  ...
  Arguments:
    targets: A list of targets in the form 'path/to/file.nnabt:target_name'.
  """
    # Keep a dict going from 'subdirectory:target_name' to 'foo.nnabt'.
    used = {}
    for target in targets:
        # Separate out 'path/to/file.nnabt, 'target_name' from
        # 'path/to/file.nnabt:target_name'.
        path, name = target.rsplit(":", 1)
        # Separate out 'path/to', 'file.nnabt' from 'path/to/file.nnabt'.
        subdir, nnabt = os.path.split(path)
        ...
        if key in used:
            ...
            raise GypError(
                ...
                'in "%s" and "%s".' % (name, subdir, nnabt, used[key])
            )
        used[key] = nnabt
...
def Load(
    ...
    # NOTE: data contains both "target" files (.nnabt) and "includes" (.nnabti), as
    ...
    if parallel:
        ...
        for build_file in build_files:
            try:
                ...
                nnabt.common.ExceptionAppend(e, "while trying to load %s" % build_file)
                raise
    ...
    if circular_check:
        # Make sure that any targets in a.nnabt don't contain dependencies in other
        # .nnabt files that further depend on a.nnabt.
