...
These functions are executed via nnabt-win-tool when using the ninja generator.
"""
...
class WinTool(object):
  ...
  be executed directly, or dispatched from an argument list."""
    ...
    a link command."""
        ...
        if manifests:
            subprocess.check_call(
                "%(python)s nnabt-win-tool manifest-wrapper %(arch)s %(mt)s -nologo "
                "-manifest %(manifests)s -out:%(out)s.manifest" % variables
            )
            if embed_manifest == "True":
                subprocess.check_call(
                    "%(python)s nnabt-win-tool manifest-to-rc %(arch)s %(out)s.manifest"
                    " %(out)s.manifest.rc %(resname)s" % variables
                )
                subprocess.check_call(
                    "%(python)s nnabt-win-tool rc-wrapper %(arch)s %(rc)s "
    ...
    for |arch|. If |dir| is supplied, use that as the working directory."""
        ...
        # through to actions that are set after nnabt-time. http://crbug.com/333738.
