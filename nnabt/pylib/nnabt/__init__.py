...
def DebugOutput(mode, message, *args):
    if "all" in nnabt.debug or mode in nnabt.debug:
...
def Load(
  ...
  """
    ...
    if format.endswith(".py"):
        ...
        generator_name = "nnabt.generator." + format
...
def RegenerateFlags(options):
  ...
  """
    ...
    def FixPath(path):
        path = nnabt.common.FixIfRelativePath(path, options.depth)
        if not path:
...
def nnabt_main(args):
    ...
    parser.add_argument(
        "--check", dest="check", action="store_true", help="check format of nnabt files"
    )
    parser.add_argument(
        "--config-dir",
        dest="config_dir",
        ...
        default=None,
        help="The location for configuration files like " "include.nnabti.",
    )
    parser.add_argument(
        "-d",
        "--debug",
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        type="path",
        help="set DEPTH nnabt variable to a relative path to PATH",
    )
    parser.add_argument(
        "-f",
        "--format",
    ...
    parser.add_argument(
        ...
        metavar="INCLUDE",
        type="path",
        help="files to include in all loaded .nnabt files",
    )
    # --no-circular-check disables the check for circular relationships between
    # .nnabt files.  These relationships should not exist, but they've only been
    # observed to be harmful with the Xcode generator.  Chromium's .nnabt files
    ...
    # Set up the configuration directory (defaults to ~/.nnabt)
    if not options.config_dir:
        home = None
        home_dot_nnabt = None
        if options.use_environment:
            home_dot_nnabt = os.environ.get("GYP_CONFIG_DIR", None)
            if home_dot_nnabt:
                home_dot_nnabt = os.path.expanduser(home_dot_nnabt)

        if not home_dot_nnabt:
            home_vars = ["HOME"]
            ...
            for home_var in home_vars:
                ...
                if home:
                    home_dot_nnabt = os.path.join(home, ".nnabt")
                    if not os.path.exists(home_dot_nnabt):
                        home_dot_nnabt = None
                    else:
                        break
    else:
        home_dot_nnabt = os.path.expanduser(options.config_dir)

    if home_dot_nnabt and not os.path.exists(home_dot_nnabt):
        home_dot_nnabt = None
    ...
    if DEBUG_GENERAL in nnabt.debug:
    ...
    # For Chromium, the nnabt "depth" variable should always be a relative path
    ...
    # If ~/.nnabt/include.nnabti exists, it'll be forcibly included into every
    # .nnabt file that's loaded, before anything else is included.
    if home_dot_nnabt:
        default_include = os.path.join(home_dot_nnabt, "include.nnabti")
    ...
    if DEBUG_GENERAL in nnabt.debug.keys():
    ...
    for format in set(options.formats):
        params = {
            ...
            "nnabt_binary": sys.argv[0],
            "home_dot_nnabt": home_dot_nnabt,
