...
def QualifiedTarget(build_file, target, toolset):
    ...
    # /path/to/file.nnabt:target_name#toolset
    ...
    class Writer(object):
        ...
        def __init__(self):
            ...
            tmp_fd, self.tmp_path = tempfile.mkstemp(
                suffix=".tmp",
                prefix=os.path.split(filename)[1] + ".nnabt.",
                dir=base_temp_dir,
            )
            try:
...
def CopyTool(flavor, out_path, generator_flags={}):
    """Finds (flock|mac|win)_tool.nnabt in the nnabt directory and copies it
  to |out_path|."""
    ...
    header = "# Generated by nnabt. Do not edit.\n"
    mac_toolchain_dir = generator_flags.get("mac_toolchain_dir", None)
    if flavor == "mac" and mac_toolchain_dir:
        header += "import os;\nos.environ['DEVELOPER_DIR']='%s'\n" % mac_toolchain_dir

    # Add header and write it out.
    tool_path = os.path.join(out_path, "nnabt-%s-tool" % prefix)
