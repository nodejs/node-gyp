...
class XcodeSettings(object):
    """A class that understands the nnabt 'xcode_settings' object."""
    ...
    def __init__(self, spec):
        ...
        # Per-target 'xcode_settings' are pushed down into configs earlier by nnabt.
    ...
    def _MapLinkerFlagFilename(self, ldflag, nnabt_to_build_path):
        """Checks if ldflag contains a filename and if so remaps it from
    nnabt-directory-relative to build-directory-relative."""
        ...
        for flag_pattern in linker_flags:
            ...
            if m:
                ldflag = (
                    ldflag[: m.start(1)]
                    + nnabt_to_build_path(m.group(1))
                    + ldflag[m.end(1) :]
                )
        # Required for ffmpeg (no idea why they don't use LIBRARY_SEARCH_PATHS,
        # TODO(thakis): Update ffmpeg.nnabt):
        if ldflag.startswith("-L"):
            ldflag = "-L" + nnabt_to_build_path(ldflag[len("-L") :])
        return ldflag

    def GetLdflags(self, configname, product_dir, nnabt_to_build_path, arch=None):
    ...
    Args:
        ...
        nnabt_to_build_path: A function that converts paths relative to the
            current nnabt file to paths relative to the build directory.
    """
        self.configname = configname
        ldflags = []

        # The xcode build is relative to a nnabt file's directory, and OTHER_LDFLAGS
        # can contain entries that depend on this. Explicitly absolutify these.
        for ldflag in self._Settings().get("OTHER_LDFLAGS", []):
            ldflags.append(self._MapLinkerFlagFilename(ldflag, nnabt_to_build_path))
        ...
        for library_path in self._Settings().get("LIBRARY_SEARCH_PATHS", []):
            ldflags.append("-L" + nnabt_to_build_path(library_path))

        if "ORDER_FILE" in self._Settings():
            ldflags.append(
                "-Wl,-order_file "
                + "-Wl,"
                + nnabt_to_build_path(self._Settings()["ORDER_FILE"])
            )
            ...
            for framework in frameworks:
                ...
                postbuilds.extend(
                    [
                        ...
                        % (
                            os.path.join("${TARGET_BUILD_DIR}", "nnabt-mac-tool"),
                            key,
    ...
    def _AdjustLibrary(self, library, config_name=None):
        ...
        # - library is referenced in the nnabt file as "$(SDKROOT)/**/*.dylib",
...
class MacPrefixHeader(object):
  ...
  * If GCC_PRECOMPILE_PREFIX_HEADER is present too, then the prefix header is
  ...
  """
    ...
    def __init__(
        self, xcode_settings, nnabt_path_to_build_path, nnabt_path_to_build_output
    ):
        """If xcode_settings is None, all methods on this class are no-ops.

    Args:
        nnabt_path_to_build_path: A function that takes a nnabt-relative path,
            and returns a path relative to the build directory.
        nnabt_path_to_build_output: A function that takes a nnabt-relative path and
        ...
        if self.header:
            if self.compile_headers:
                for lang in ["c", "cc", "m", "mm"]:
                    self.compiled_headers[lang] = nnabt_path_to_build_output(
                        self.header, lang
                    )
            self.header = nnabt_path_to_build_path(self.header)
...
def GetMacInfoPlist(product_dir, xcode_settings, nnabt_path_to_build_path):
  ...
  * |extra_env| is a dict of env variables that should be exported when
  ...
  """
    ...
    info_plist = nnabt_path_to_build_path(info_plist)
...
def _TopologicallySortedEnvVarKeys(env):
  ...
  """
    ...
    try:
        ...
        # above).
        order = nnabt.common.TopologicallySorted(env.keys(), GetEdges)
        order.reverse()
        return order
    except nnabt.common.CycleError as e:
        raise GypError(
...
def GetSpecPostbuildCommands(spec, quiet=False):
  ...
  executable by a shell."""
    ...
    for postbuild in spec.get("postbuilds", []):
        ...
        postbuilds.append(nnabt.common.EncodePOSIXShellList(postbuild["action"]))
    return postbuilds
