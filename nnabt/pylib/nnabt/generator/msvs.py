...
import nnabt.common
import nnabt.easy_xml as easy_xml
import nnabt.generator.ninja as ninja_generator
import nnabt.MSVSNew as MSVSNew
import nnabt.MSVSProject as MSVSProject
import nnabt.MSVSSettings as MSVSSettings
import nnabt.MSVSToolFile as MSVSToolFile
import nnabt.MSVSUserFile as MSVSUserFile
import nnabt.MSVSUtil as MSVSUtil
import nnabt.MSVSVersion as MSVSVersion
...
generator_supports_multiple_toolsets = nnabt.common.CrossCompileRequested()
...
def _BuildCommandLineForRuleRaw(
    ...
    if cygwin_shell:
        ...
        # direct_cmd = nnabt.common.EncodePOSIXShellList(direct_cmd)
...
def _GenerateNativeRulesForMSVS(p, rules, output_dir, spec, options):
  ...
  Arguments:
    ...
    output_dir: the directory in which the project/nnabt resides
...
def _GenerateExternalRules(rules, output_dir, spec, sources, options, actions_to_add):
  ...
  Arguments:
    ...
    output_dir: path containing project and nnabt files
  ...
  """
    ...
    mk_file = nnabt.common.WriteOnDiff(os.path.join(output_dir, filename))
...
def _GenerateMSVSProject(project, options, version, generator_flags):
  ...
  """
    spec = project.spec
    nnabt.common.EnsureDirExists(project.path)
    ...
    nnabt_path = _NormalizedSource(project.build_file)
    relative_path_of_nnabt_file = nnabt.common.RelativePath(nnabt_path, project_dir)
    ...
    nnabt_file = os.path.split(project.build_file)[1]
    sources, excluded_sources = _PrepareListOfSources(spec, generator_flags, nnabt_file)
    ...
    _AddActions(actions_to_add, spec, relative_path_of_nnabt_file)
...
def _GetMSVSConfigurationType(spec, build_file):
  ...
  Args:
      spec: The target dictionary containing the properties of the target.
      build_file: The path of the nnabt file.
  Returns:
...
def _PrepareListOfSources(spec, generator_flags, nnabt_file):
    """Prepare list of sources and excluded sources.

  Besides the sources specified directly in the spec, adds the nnabt file so
  ...
  Arguments:
    spec: The target dictionary containing the properties of the target.
    nnabt_file: The name of the nnabt file.
  Returns:
    A pair of (list of sources, list of excluded sources).
    The sources will be relative to the nnabt file.
  """
    sources = OrderedSet()
    _AddNormalizedSources(sources, spec.get("sources", []))
    excluded_sources = OrderedSet()
    # Add in the nnabt file.
    if not generator_flags.get("standalone"):
        sources.add(nnabt_file)
...
def _AdjustSourcesAndConvertToFilterHierarchy(
    spec, options, nnabt_dir, sources, excluded_sources, list_excluded, version
):
  ...
  Arguments:
    spec: The target dictionary containing the properties of the target.
    options: Global generator options.
    nnabt_dir: The path to the nnabt file being processed.
...
def _AddActions(actions_to_add, spec, relative_path_of_nnabt_file):
    # Add actions.
    ...
    have_setup_env = set()
    for a in actions:
        # Attach actions to the nnabt file if nothing else is there.
        inputs = a.get("inputs") or [relative_path_of_nnabt_file]
...
def _GatherSolutionFolders(sln_projects, project_objects, flat):
    ...
    for p in sln_projects:
        nnabt_file, target = nnabt.common.ParseQualifiedTarget(p)[0:2]
        if p.endswith("#host"):
            target += "_host"
        nnabt_dir = os.path.dirname(nnabt_file)
        path_dict = _GetPathDict(root, nnabt_dir)
...
def _GetPathOfProject(qualified_target, spec, options, msvs_version):
    ...
    build_file = nnabt.common.BuildFile(qualified_target)
    proj_path = os.path.join(os.path.dirname(build_file), proj_filename)
    fix_prefix = None
    if options.generator_output:
        project_dir_path = os.path.dirname(os.path.abspath(proj_path))
        proj_path = os.path.join(options.generator_output, proj_path)
        fix_prefix = nnabt.common.RelativePath(
...
def _InitNinjaFlavor(params, target_list, target_dicts):
  ...
  Arguments:
    ...
    target_list: List of target pairs: 'base/base.nnabt:base'.
    ...
    for qualified_target in target_list:
        ...
        if not spec.get("msvs_external_builder_out_dir"):
            ...
            spec["msvs_external_builder_out_dir"] = os.path.join(
                nnabt.common.RelativePath(params["options"].toplevel_dir, nnabt_dir),
...
def CalculateVariables(default_variables, params):
    ...
    if nnabt.common.GetFlavor(params) == "ninja":
...
def GenerateOutput(target_list, target_dicts, data, params):
  ...
  Arguments:
    target_list: List of target pairs: 'base/base.nnabt:base'.
    target_dicts: Dict of target properties keyed on target pair.
    data: Dictionary containing per .nnabt data.
  """
    global fixpath_prefix
    ...
    for build_file in data:
        ...
        if not build_file.endswith(".nnabt"):
            continue
        sln_path = os.path.splitext(build_file)[0] + options.suffix + ".sln"
        if options.generator_output:
            sln_path = os.path.join(options.generator_output, sln_path)
        # Get projects in the solution, and their dependents.
        sln_projects = nnabt.common.BuildFileTargets(target_list, build_file)
        sln_projects += nnabt.common.DeepDependencyTargets(target_dicts, sln_projects)
...
def _GetMSBuildGlobalProperties(spec, version, guid, nnabt_file_name):
    namespace = os.path.splitext(nnabt_file_name)[0]
    properties = [
        [
            "PropertyGroup",
            {"Label": "Globals"},
...
def _GetMSBuildPropertyGroup(spec, label, properties):
  ...
  """
    ...
    properties_ordered = nnabt.common.TopologicallySorted(properties.keys(), GetEdges)
...
def _GetMSBuildProjectReferences(project):
    ...
    if project.dependencies:
        ...
        for dependency in project.dependencies:
            ...
            project_dir = os.path.split(project.path)[0]
            relative_path = nnabt.common.RelativePath(dependency.path, project_dir)
            project_ref = [
                "ProjectReference",
