...
import nnabt
import nnabt.common
import nnabt.xcode_emulation
from nnabt.common import GetEnvironFallback
from nnabt.common import GypError
...
def CalculateVariables(default_variables, params):
    """Calculate additional variables for use in the build (called by nnabt)."""
    flavor = nnabt.common.GetFlavor(params)
    if flavor == "mac":
        ...
        import nnabt.generator.xcode as xcode_generator
...
def CalculateGeneratorInputInfo(params):
  ...
  nnabt)."""
    ...
    qualified_out_dir = os.path.normpath(
        os.path.join(output_dir, builddir_name, "nnabtfiles")
    )
...
cmd_alink = rm -f $@ && ./nnabt-mac-tool filter-libtool libtool $(GYP_LIBTOOLFLAGS) -static -o $@ $(filter %.o,$^)
...
# TODO(evan): move all cross-compilation logic to nnabt-time so we don't need
...
# nnabt-mac-tool is written next to the root Makefile by nnabt.
# Use $(4) for the command, since $(2) and $(3) are used as flag by do_cmd
# already.
quiet_cmd_mac_tool = MACTOOL $(4) $<
cmd_mac_tool = ./nnabt-mac-tool $(4) $< "$@"

quiet_cmd_mac_package_framework = PACKAGE FRAMEWORK $@
cmd_mac_package_framework = ./nnabt-mac-tool package-framework "$@" $(4)
...
header = """\
# This file is generated by nnabt; do not edit.
...
"""
    ...
    Arguments:
      ...
      spec, configs: nnabt info
      part_of_all: flag indicating this target is part of 'all'
    """
        nnabt.common.EnsureDirExists(output_filename)
        ...
        self.is_mac_bundle = nnabt.xcode_emulation.IsMacBundle(self.flavor, spec)
        if self.flavor == "mac":
            self.xcode_settings = nnabt.xcode_emulation.XcodeSettings(spec)
        else:
        ...
        if all_sources:
            ...
            self.WriteSources(
                ...
                extra_link_deps,
                part_of_all,
                nnabt.xcode_emulation.MacPrefixHeader(
                    self.xcode_settings,
    ...
    the targets from a single nnabt file (i.e. a sub-project).
    ...
    """
        ...
        # sub-project dir (see test/subdirectory/nnabttest-subdir-all.py).
        self.WriteLn(
    ...
    def WriteActions(
        ...
        """Write Makefile code for any 'actions' from the nnabt input.
    ...
    """
        ...
        for action in actions:
            ...
            if self.flavor == "mac":
                action_commands = [
                    nnabt.xcode_emulation.ExpandEnvVars(command, env)
                    for command in action_commands
                ]
            command = nnabt.common.EncodePOSIXShellList(action_commands)
            ...
            # The makefile rules are all relative to the top dir, but the nnabt actions
            ...
            outputs = [nnabt.xcode_emulation.ExpandEnvVars(o, env) for o in outputs]
            inputs = [nnabt.xcode_emulation.ExpandEnvVars(i, env) for i in inputs]
    ...
    def WriteRules(
        ...
        """Write Makefile code for any 'rules' from the nnabt input.
    ...
    """
        ...
        for rule in rules:
            ...
            for rule_source in rule.get("rule_sources", []):
                ...
                action = nnabt.common.EncodePOSIXShellList(action)
    ...
    def WriteCopies(self, copies, extra_outputs, part_of_all):
        """Write Makefile code for any 'copies' from the nnabt input.
    ...
    """
        ...
        for copy in copies:
            for path in copy["files"]:
                ...
                # As a workaround, manually expand variables at nnabt time. Since 'copies'
                # can't run scripts, there's no need to write the env then.
                # WriteDoCmd() will escape spaces for .d files.
                env = self.GetSortedXcodeEnv()
                output = nnabt.xcode_emulation.ExpandEnvVars(output, env)
                path = nnabt.xcode_emulation.ExpandEnvVars(path, env)
    ...
    def WriteMacBundleResources(self, resources, bundle_deps):
        ...
        for output, res in nnabt.xcode_emulation.GetMacBundleResources(
    ...
    def WriteMacInfoPlist(self, bundle_deps):
        ...
        info_plist, out, defines, extra_env = nnabt.xcode_emulation.GetMacInfoPlist(
            generator_default_variables["PRODUCT_DIR"],
            self.xcode_settings,
            lambda p: Sourceify(self.Absolutify(p)),
        )
    ...
    def WriteSources(
        ...
        """Write Makefile code for any 'sources' from the nnabt input.
    These are source files necessary to build the current target.

    configs, deps, sources: input from nnabt.
...
# See "Target-specific Variable Values" in the GNU Make manual."""
    ...
    def ComputeOutputBasename(self, spec):
        """Return the 'output basename' of a nnabt spec.
    ...
    def ComputeOutput(self, spec):
        """Return the 'output' (full output path) of a nnabt spec.
    ...
    def ComputeDeps(self, spec):
        """Compute the dependencies of a nnabt spec.
    ...
    """
        ...
        return (nnabt.common.uniquer(deps), nnabt.common.uniquer(link_deps))
    ...
    def WriteTarget(
        ...
        """Write Makefile code to produce the final target of the nnabt spec.

    spec, configs: input from nnabt.
    ...
    """
        ...
        if self.type != "none":
            for configname in sorted(configs.keys()):
                ...
                if self.flavor == "mac":
                    ...
                    nnabt_to_build = nnabt.common.InvertRelativePath(self.path)
                    target_postbuild = self.xcode_settings.AddImplicitPostbuilds(
                        configname,
                        QuoteSpaces(
                            os.path.normpath(os.path.join(nnabt_to_build, self.output))
                        ),
                        QuoteSpaces(
                            os.path.normpath(
                                os.path.join(nnabt_to_build, self.output_binary)
                            )
                        ),
                    )
                    if target_postbuild:
            ...
            if libraries:
                ...
                libraries = nnabt.common.uniquer(libraries)
                if self.flavor == "mac":
        ...
        if self.flavor == "mac":
            ...
            postbuilds.extend(nnabt.xcode_emulation.GetSpecPostbuildCommands(spec))
            ...
            for configname in target_postbuilds:
                self.WriteLn(
                    ...
                    % (
                        ...
                        configname,
                        nnabt.common.EncodePOSIXShellList(target_postbuilds[configname]),
                    )
                )

            # Postbuilds expect to be run in the nnabt file's directory, so insert an
            # implicit postbuild to cd to there.
            postbuilds.insert(0, nnabt.common.EncodePOSIXShellList(["cd", self.path]))
        ...
        if self.is_mac_bundle:
            ...
            # Needed by test/mac/nnabttest-rebuild.py.
    ...
    def GetSortedXcodeEnv(self, additional_settings=None):
        return nnabt.xcode_emulation.GetSortedXcodeEnv(
            self.xcode_settings,
            "$(abs_builddir)",
    ...
    def _InstallableTargetInstallPath(self):
        ...
        # Xcode puts shared_library results into PRODUCT_DIR, and some nnabt files
...
def WriteAutoRegenerationRule(params, root_makefile, makefile_name, build_files):
    ...
    build_files_args = [
        nnabt.common.RelativePath(filename, options.toplevel_dir)
        for filename in params["build_files_arg"]
    ]

    nnabt_binary = nnabt.common.FixIfRelativePath(
        params["nnabt_binary"], options.toplevel_dir
    )
    if not nnabt_binary.startswith(os.sep):
        nnabt_binary = os.path.join(".", nnabt_binary)
    ...
    root_makefile.write(
        ...
        % {
            ...
            "cmd": nnabt.common.EncodePOSIXShellList(
                [nnabt_binary, "-fmake"] + nnabt.RegenerateFlags(options) + build_files_args
            ),
        }
    )
...
def GenerateOutput(target_list, target_dicts, data, params):
    ...
    def CalculateMakefilePath(build_file, base_name):
        """Determine where to write a Makefile for a given nnabt file."""
        # Paths in nnabt files are relative to the .nnabt file, but we want
        # paths relative to the source root for the master makefile.  Grab
        # the path of the .nnabt file as the base to relativize against.
        # E.g. "foo/bar" when we're constructing targets for "foo/bar/baz.nnabt".
        base_path = nnabt.common.RelativePath(os.path.dirname(build_file), options.depth)
        ...
        base_path = nnabt.common.RelativePath(
    ...
    if options.generator_output:
        ...
        srcdir = nnabt.common.RelativePath(srcdir, options.generator_output)
    ...
    if flavor == "mac":
        flock_command = "./nnabt-mac-tool flock"
    ...
    build_file, _, _ = nnabt.common.ParseQualifiedTarget(target_list[0])
    ...
    for key, value in make_global_settings_array:
        ...
        if key in ("CC", "CC.host", "CXX", "CXX.host"):
            ...
            # Let nnabt-time envvars win over global settings.
    ...
    nnabt.common.EnsureDirExists(makefile_path)
    ...
    nnabt.common.CopyTool(flavor, dest_path)

    # Find the list of targets that derive from the nnabt file(s) being built.
    needed_targets = set()
    for build_file in params["build_files"]:
        for target in nnabt.common.AllTargets(target_list, target_dicts, build_file):
            needed_targets.add(target)

    build_files = set()
    include_list = set()
    for qualified_target in target_list:
        build_file, target, toolset = nnabt.common.ParseQualifiedTarget(qualified_target)
        ...
        build_files.add(nnabt.common.RelativePath(build_file, options.toplevel_dir))
        included_files = data[build_file]["included_files"]
        for included_file in included_files:
            # The included_files entries are relative to the dir of the build file
            # that included them, so we have to undo that and then make them relative
            # to the root dir.
            relative_include_file = nnabt.common.RelativePath(
                nnabt.common.UnrelativePath(included_file, build_file),
                options.toplevel_dir,
            )
            abs_include_file = os.path.abspath(relative_include_file)
            # If the include file is from the ~/.nnabt dir, we should use absolute path
            # so that relocating the src dir doesn't break the path.
            if params["home_dot_nnabt"] and abs_include_file.startswith(
                params["home_dot_nnabt"]
            ):
        ...
        if flavor == "mac":
            nnabt.xcode_emulation.MergeGlobalXcodeSettingsToSpec(data[build_file], spec)
        ...
        mkfile_rel_path = nnabt.common.RelativePath(
            output_file, os.path.dirname(makefile_path)
        )
        include_list.add(mkfile_rel_path)

    # Write out per-nnabt (sub-project) Makefiles.
    depth_rel_path = nnabt.common.RelativePath(options.depth, os.getcwd())
    for build_file in build_files:
        ...
        # Only generate Makefiles for nnabt files with targets.
        if not nnabt_targets:
            continue
        base_path, output_file = CalculateMakefilePath(
            build_file, os.path.splitext(os.path.basename(build_file))[0] + ".Makefile"
        )
        makefile_rel_path = nnabt.common.RelativePath(
            os.path.dirname(makefile_path), os.path.dirname(output_file)
        )
        writer.WriteSubMake(output_file, makefile_rel_path, nnabt_targets, builddir_name)
