...
for dirname in ["INTERMEDIATE_DIR", "PRODUCT_DIR", "LIB_DIR", "SHARED_LIB_DIR"]:
    # Some nnabt steps fail if these are empty(!), so we convert them to variables
  ...
  """
    ...
    for target_name in target_list:
        ...
        if config_name in target["configurations"]:
            ...
            # may be done in nnabt files to force certain includes to come at the end.
            # TODO(jgreenwald): Change the nnabt files to not abuse cflags for this, and
            # remove this.
            if flavor == "win":
                msvs_settings = nnabt.msvs_emulation.MsvsSettings(target, generator_flags)
            ...
            # Find standard nnabt include dirs.
            if "include_dirs" in config:
                ...
                for shared_intermediate_dir in shared_intermediate_dirs:
                    for include_dir in include_dirs:
                        ...
                        nnabt_includes_set.add(include_dir)

    # Generate a list that has all the include dirs.
    all_includes_list = list(nnabt_includes_set)
    all_includes_list.sort()
    for compiler_include in compiler_includes_list:
        if compiler_include not in nnabt_includes_set:
...
def GetCompilerPath(target_list, data, options):
  ...
  Returns:
    If this is a nnabt project that has explicit make settings, try to determine
...
def GetAllDefines(target_list, target_dicts, data, config_name, params, compiler_path):
  ...
  Returns:
    A dict that includes explicit defines declared in nnabt files along with all
    of the default defines that the compiler uses.
  """

    # Get defines declared in the nnabt files.
    all_defines = {}
    flavor = nnabt.common.GetFlavor(params)
    if flavor == "win":
        generator_flags = params.get("generator_flags", {})
...
def GenerateCdtSettingsFile(
    ...
    nnabt.common.EnsureDirExists(out_name)
