...
to nnabt. For example, if file "a.nnabt" contains targets "a1" and
"a2", and file "b.nnabt" contains targets "b1" and "b2" and "a2" has a dependency
on "b2" and nnabt is supplied "a.nnabt" then "all" consists of "a1" and "a2".
Notice that "b1" and "b2" are not in the "all" target as "b.nnabt" was not
directly supplied to nnabt. OTOH if both "a.nnabt" and "b.nnabt" are supplied to nnabt
...
def _ToGypPath(path):
    """Converts a path to the format used by nnabt."""
...
def _AddSources(sources, base_path, base_path_components, result):
  ...
  and tracked in some other means."""
    # NOTE: nnabt paths are always posix style.
...
def _WasBuildFileModified(build_file, data, files, toplevel_dir):
  ...
  the root of the source tree."""
    if _ToLocalPath(toplevel_dir, _ToGypPath(build_file)) in files:
        if debug:
            print("nnabt file modified", build_file)
        return True
    ...
    for include_file in data[build_file]["included_files"][1:]:
        ...
        rel_include_file = _ToGypPath(
            nnabt.common.UnrelativePath(include_file, build_file)
        )
        if _ToLocalPath(toplevel_dir, rel_include_file) in files:
            if debug:
                print(
                    "included nnabt file modified, nnabt_file=",
                    build_file,
                    "included file=",
                    rel_include_file,
                )
...
def _GenerateTargets(data, target_list, target_dicts, toplevel_dir, files, build_files):
  ...
  |toplevel_dir| is the root of the source tree."""
    ...
    while len(targets_to_visit) > 0:
        ...
        build_file = nnabt.common.ParseQualifiedTarget(target_name)[0]
...
def _GetUnqualifiedToTargetMapping(all_targets, to_find):
  ...
  . any target names not found. If this is empty all targets were found."""
    ...
    for target_name in all_targets.keys():
        extracted = nnabt.common.ParseQualifiedTarget(target_name)
...
def CalculateVariables(default_variables, params):
    ...
    if flavor == "mac":
        ...
        nnabt.msvs_emulation.CalculateCommonVariables(default_variables, params)
    else:
...
class TargetCalculator(object):
    ...
    def find_matching_test_target_names(self):
        ...
        matching_target_names = [
            nnabt.common.ParseQualifiedTarget(target.name)[1]
            for target in matching_test_targets
        ]
        if matching_test_targets_contains_all:
...
def GenerateOutput(target_list, target_dicts, data, params):
    """Called by nnabt as the final stage. Outputs results."""
    config = Config()
    try:
        config.Init(params)
