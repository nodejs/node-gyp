...
import filecmp
import nnabt.common
import nnabt.xcodeproj_file
import nnabt.xcode_ninja
import errno
import os
import sys
import posixpath
...
def CreateXCConfigurationList(configuration_names):
    xccl = nnabt.xcodeproj_file.XCConfigurationList({"buildConfigurations": []})
    if len(configuration_names) == 0:
        configuration_names = ["Default"]
    for configuration_name in configuration_names:
        xcbc = nnabt.xcodeproj_file.XCBuildConfiguration({"name": configuration_name})
...
class XcodeProject(object):
    def __init__(self, nnabt_path, path, build_file_dict):
        self.nnabt_path = nnabt_path
        self.path = path
        self.project = nnabt.xcodeproj_file.PBXProject(path=path)
        projectDirPath = nnabt.common.RelativePath(
            os.path.dirname(os.path.abspath(self.nnabt_path)),
            os.path.dirname(path) or ".",
        )
        self.project.SetProperty("projectDirPath", projectDirPath)
        self.project_file = nnabt.xcodeproj_file.XCProjectFile(
    ...
    def Finalize1(self, xcode_targets, serialize_all_tests):
        ...
        try:
            ...
            sys.stderr.write("Problem with nnabt file %s\n" % self.nnabt_path)
            raise
        ...
        for target in self.build_file_dict["targets"]:
            ...
            qualified_target = nnabt.common.QualifiedTarget(
                self.nnabt_path, target_name, toolset
            )
            ...
            if target.get("run_as"):
                ...
                xccl = CreateXCConfigurationList(configurations)
                run_target = nnabt.xcodeproj_file.PBXAggregateTarget(
                    {
                ...
                if command.get("working_directory"):
                    script = (
                        script
                        + 'cd "%s"\n'
                        % nnabt.xcodeproj_file.ConvertVariablesToShellSyntax(
...
sys.exit(subprocess.call(sys.argv[1:]))" """
                ...
                script = (
                    ...
                    + "%s\nexit 1\n"
                    % nnabt.xcodeproj_file.ConvertVariablesToShellSyntax(
                        nnabt.common.EncodePOSIXShellList(command.get("action"))
                    )
                )

                ssbp = nnabt.xcodeproj_file.PBXShellScriptBuildPhase(
        ...
        if len(targets_for_all) > 1 and not has_custom_all:
            xccl = CreateXCConfigurationList(configurations)
            all_target = nnabt.xcodeproj_file.PBXAggregateTarget(
        ...
        if len(run_test_targets) > 1:
            xccl = CreateXCConfigurationList(configurations)
            run_all_tests_target = nnabt.xcodeproj_file.PBXAggregateTarget(
    ...
    def Finalize2(self, xcode_targets, xcode_target_to_target_dict):
        ...
        for bf_tgt in self.build_file_dict["targets"]:
            if int(bf_tgt.get("xcode_create_dependents_test_runner", 0)):
                ...
                qualified_target = nnabt.common.QualifiedTarget(
                    self.nnabt_path, tgt_name, toolset
                )
                xcode_target = xcode_targets[qualified_target]
                if isinstance(xcode_target, nnabt.xcodeproj_file.PBXAggregateTarget):
                    ...
                    if len(all_run_tests) > 0:
                        run_all_target = nnabt.xcodeproj_file.PBXAggregateTarget(
                            {
    ...
    def Write(self):
        ...
        (output_fd, new_pbxproj_path) = tempfile.mkstemp(
            suffix=".tmp", prefix="project.pbxproj.nnabt.", dir=self.path
        )
...
def CalculateGeneratorInputInfo(params):
    ...
    if params.get("flavor") == "ninja":
        ...
        qualified_out_dir = os.path.normpath(
            os.path.join(toplevel, output_dir, "nnabtfiles-xcode-ninja")
        )
    else:
        output_dir = os.path.normpath(os.path.join(toplevel, "xcodebuild"))
        qualified_out_dir = os.path.normpath(
            os.path.join(toplevel, output_dir, "nnabtfiles")
        )
...
def GenerateOutput(target_list, target_dicts, data, params):
    ...
    if ninja_wrapper:
        (target_list, target_dicts, data) = nnabt.xcode_ninja.CreateWrapper(
    ...
    for build_file, build_file_dict in data.items():
        ...
        # Add nnabt/nnabti files to project
        if not generator_flags.get("standalone"):
            main_group = pbxp.GetProperty("mainGroup")
            build_group = nnabt.xcodeproj_file.PBXGroup({"name": "Build"})
    ...
    for qualified_target in target_list:
        ...
        # com.googlecode.nnabt.xcode.bundle, a pseudo-type that xcode.py interprets
        # to create a single-file mh_bundle.
        _types = {
            "executable": "com.apple.product-type.tool",
            "loadable_module": "com.googlecode.nnabt.xcode.bundle",
        ...
        if type != "none":
            ...
            xctarget_type = nnabt.xcodeproj_file.PBXNativeTarget
            try:
            ...
            xctarget_type = nnabt.xcodeproj_file.PBXAggregateTarget
        ...
        if type != "none" and (spec_actions or spec_rules) and not ninja_wrapper:
            ...
            support_xct = nnabt.xcodeproj_file.PBXAggregateTarget(
        ...
        for action in spec_actions:
            ...
            if message:
                message = "echo note: " + nnabt.common.EncodePOSIXShellArgument(message)
            else:
                message = ""

            # Turn the list into a string that can be passed to a shell.
            action_string = nnabt.common.EncodePOSIXShellList(action["action"])

            # Convert Xcode-type variable references to sh-compatible environment
            # variable references.
            message_sh = nnabt.xcodeproj_file.ConvertVariablesToShellSyntax(message)
            action_string_sh = nnabt.xcodeproj_file.ConvertVariablesToShellSyntax(
                action_string
            )
        ...
        for rule in spec_rules:
            ...
            for rule_source in rule.get("rule_sources", []):
                ...
                if message:
                    message = nnabt.common.EncodePOSIXShellArgument(message)
                    message = ExpandXcodeVariables(message, rule_input_dict)
                messages.append(message)

                # Turn the list into a string that can be passed to a shell.
                action_string = nnabt.common.EncodePOSIXShellList(rule["action"])
...
"""
        ...
        for copy_group in spec.get("copies", []):
            ...
            if pbxcp is None:
                pbxcp = nnabt.xcodeproj_file.PBXCopyFilesBuildPhase(
                    {"name": "Copy to " + copy_group["destination"]}, parent=xct
                )
                pbxcp.SetDestination(dest)

                # TODO(mark): The usual comment about this knowing too much about
                # nnabt.xcodeproj_file internals applies.
        ...
        for postbuild in spec.get("postbuilds", []):
            action_string_sh = nnabt.common.EncodePOSIXShellList(postbuild["action"])
            script = "exec " + action_string_sh + "\nexit 1\n"
