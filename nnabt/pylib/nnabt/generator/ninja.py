...
import nnabt.ninja_syntax as ninja_syntax
...
generator_default_variables = {
    ...
    # known at nnabt time.  To resolve this, introduce special
    # variables starting with $! and $| (which begin with a $ so nnabt knows it
    # should be treated specially, but is otherwise an invalid
    # ninja/shell variable) that are passed to nnabt here but expanded
    ...
    # Special variables that may be used by nnabt 'rule' targets.
...
def QuoteShellArgument(arg, flavor):
  ...
  by the shell."""
    ...
    if flavor == "win":
        return nnabt.msvs_emulation.QuoteForRspFile(arg)
    return "'" + arg.replace("'", "'" + '"\'"' + "'") + "'"
...
class Target(object):
    """Target represents the paths used within a single nnabt target.
...
# All files we produce (both at nnabt and at build time) appear in the
# build directory (e.g. out/Debug).
#
# Paths within a given .nnabt file are always relative to the directory
# containing the .nnabt file.  Call these "nnabt paths".  This includes
# sources as well as the starting directory a given nnabt rule/action
# expects to be run from.  We call the path from the source root to
# the nnabt file the "base directory" within the per-.nnabt-file
# NinjaWriter code.
#
...
# functions:
#
# - GypPathToNinja translates a nnabt path (i.e. relative to the .nnabt file)
#   into the equivalent ninja path.
#
# - GypPathToUniqueOutput translates a nnabt path into a ninja path to write
...
class NinjaWriter(object):
    def __init__(
        ...
        """
    base_dir: path from source root to directory containing this nnabt file,
              by nnabt semantics, all input paths are relative to this
    ...
    """
        ...
        build_to_top = nnabt.common.InvertRelativePath(build_dir, toplevel_dir)
        self.build_to_base = os.path.join(build_to_top, base_dir)
        # Relative path from base dir to build dir.
        base_to_top = nnabt.common.InvertRelativePath(base_dir, toplevel_dir)
    ...
    def GypPathToNinja(self, path, env=None):
        """Translate a nnabt path to a ninja path, optionally expanding environment
    ...
    See the above discourse on path conversions."""
        if env:
            if self.flavor == "mac":
                ...
                path = nnabt.msvs_emulation.ExpandMacros(path, env)
        if path.startswith("$!"):
    ...
    def GypPathToUniqueOutput(self, path, qualified=True):
        """Translate a nnabt path to a ninja path for writing output.
    ...
    See the above discourse on path conversions."""
        ...
        #   Input: foo/bar.nnabt, target targ, references baz/out.o
        ...
        #    its path, even if the input is brought via a nnabt file with '..'.
    ...
    target)."""
        ...
        if self.flavor == "win":
            self.msvs_settings = nnabt.msvs_emulation.MsvsSettings(spec, generator_flags)
        ...
        if sources:
            ...
            pch = None
            if self.flavor == "win":
                nnabt.msvs_emulation.VerifyMissingSources(
                    sources, self.abs_build_dir, generator_flags, self.GypPathToNinja
                )
                pch = nnabt.msvs_emulation.PrecompiledHeader(
                    self.msvs_settings,
                    config_name,
                    self.GypPathToNinja,
                ...
                pch = nnabt.xcode_emulation.MacPrefixHeader(
                    self.xcode_settings,
                    self.GypPathToNinja,
    ...
    |fallback| is the nnabt-level name of the step, usable as a fallback.
    """
        if self.toolset != "target":
            verb += "(%s)" % self.toolset
        if message:
    ...
    def WriteCopies(self, copies, prebuild, mac_bundle_depends):
        ...
        for to_copy in copies:
            for path in to_copy["files"]:
                ...
                if self.is_mac_bundle:
                    # nnabt has mac_bundle_resources to copy things into a bundle's
    ...
    def WriteLink(self, spec, config_name, config, link_deps, compile_deps):
        ...
        if self.flavor != "mac" or len(self.archs) == 1:
            ...
            # https://code.google.com/p/nnabt/issues/detail?id=411
            if (
    ...
    def WriteLinkForArch(
        ...
        libraries = nnabt.common.uniquer(
        ...
        if command in ("solink", "solink_module"):
            ...
            extra_bindings.append(("lib", nnabt.common.EncodePOSIXShellArgument(output)))
            if self.flavor != "win":
                ...
                extra_bindings.append(
                    (
                        "link_file_list",
                        nnabt.common.EncodePOSIXShellArgument(link_file_list),
                    )
                )
            if self.flavor == "win":
        ...
        if len(solibs):
            extra_bindings.append(
                ("solibs", nnabt.common.EncodePOSIXShellList(sorted(solibs)))
            )
    ...
    returned string will start with ' && '."""
        ...
        postbuilds = nnabt.xcode_emulation.GetSpecPostbuildCommands(spec, quiet=True)
        ...
        postbuilds.insert(
            0, nnabt.common.EncodePOSIXShellList(["cd", self.build_to_base])
        )
    ...
    that exports |env| to the shell."""
        ...
        for k, v in env:
            export_str.append(
                "export %s=%s;"
                % (k, ninja_syntax.escape(nnabt.common.EncodePOSIXShellArgument(v)))
            )
    ...
    expanded."""
        ...
        if self.flavor == "win":
            ...
            args = [nnabt.xcode_emulation.ExpandEnvVars(arg, env) for arg in args]
            description = nnabt.xcode_emulation.ExpandEnvVars(description, env)
        ...
        # nnabt dictates that commands are run from the base directory.
        ...
        if self.flavor == "win":
            ...
            if is_cygwin:
                ...
                rspfile_content = nnabt.msvs_emulation.EncodeRspFileList(args)
            command = (
                "%s nnabt-win-tool action-wrapper $arch " % sys.executable
                + rspfile
                + run_in
            )
        else:
            env = self.ComputeExportEnvString(env)
            command = nnabt.common.EncodePOSIXShellList(args)
...
def CalculateVariables(default_variables, params):
    ...
    if flavor == "mac":
        ...
        exts = nnabt.MSVSUtil.TARGET_TYPE_EXT
        ...
        import nnabt.generator.msvs as msvs_generator
...
def CalculateGeneratorInputInfo(params):
    ...
    qualified_out_dir = os.path.normpath(
        os.path.join(toplevel, ComputeOutputDir(params), "nnabtfiles")
    )
...
def _AddWinLinkRules(master_ninja, embed_manifest):
    ...
    def FullLinkCommand(ldcmd, out, binary_type):
        ...
        return (
            "%(python)s nnabt-win-tool link-with-manifests $arch %(embed)s "
    ...
    dllcmd = (
        "%s nnabt-win-tool link-wrapper $arch %s "
...
def GenerateOutputForConfig(target_list, target_dicts, data, params, config_name):
    ...
    nnabt.common.CopyTool(flavor, toplevel_build, generator_flags)

    # Grab make settings for CC/CXX.
    # The rules are
    # - The priority from low to high is gcc/g++, the 'make_global_settings' in
    #   nnabt, the environment variable.
    ...
    build_to_root = nnabt.common.InvertRelativePath(build_dir, options.toplevel_dir)
    wrappers = {}
    ...
    if flavor == "win":
        ...
        if not generator_flags.get("ninja_use_custom_environment_files", 0):
            shared_system_includes = nnabt.msvs_emulation.ExtractSharedMSVSSystemIncludes(
                configs, generator_flags
            )
        cl_paths = nnabt.msvs_emulation.GenerateEnvironmentFiles(
    ...
    if flavor != "win":
        ...
        master_ninja.rule(
            ...
            description="IDL $in",
            command=(
                "%s nnabt-win-tool midl-wrapper $arch $outdir "
    ...
    if flavor != "mac" and flavor != "win":
        ...
        master_ninja.rule(
            ...
            description="LIB $out",
            command=(
                "%s nnabt-win-tool link-wrapper $arch False "
        ...
        master_ninja.rule(
            ...
            command="rm -f $out && "
            "./nnabt-mac-tool filter-libtool libtool $libtool_flags "
            "-static -o $out $in"
            "$postbuilds",
        )
        master_ninja.rule(
        ...
        master_ninja.rule(
            ...
            command="$env ./nnabt-mac-tool copy-info-plist $in $out $binary $keys",
        )
        master_ninja.rule(
            "merge_infoplist",
            description="MERGE INFOPLISTS $in",
            command="$env ./nnabt-mac-tool merge-info-plist $out $in",
        )
        master_ninja.rule(
            "compile_xcassets",
            description="COMPILE XCASSETS $in",
            command="$env ./nnabt-mac-tool compile-xcassets $keys $in",
        )
        master_ninja.rule(
            "compile_ios_framework_headers",
            description="COMPILE HEADER MAPS AND COPY FRAMEWORK HEADERS $in",
            command="$env ./nnabt-mac-tool compile-ios-framework-header-map $out "
            "$framework $in && $env ./nnabt-mac-tool "
            "copy-ios-framework-headers $framework $copy_headers",
        )
        master_ninja.rule(
            "mac_tool",
            description="MACTOOL $mactool_cmd $in",
            command="$env ./nnabt-mac-tool $mactool_cmd $in $out $binary",
        )
        master_ninja.rule(
            "package_framework",
            description="PACKAGE FRAMEWORK $out, POSTBUILDS",
            command="./nnabt-mac-tool package-framework $out $version$postbuilds "
            "&& touch $out",
        )
        master_ninja.rule(
            "package_ios_framework",
            description="PACKAGE IOS FRAMEWORK $out, POSTBUILDS",
            command="./nnabt-mac-tool package-ios-framework $out $postbuilds "
            "&& touch $out",
        )
    if flavor == "win":
        master_ninja.rule(
            ...
            command="%s nnabt-win-tool stamp $out" % sys.executable,
        )
    else:
        master_ninja.rule(
    ...
    if flavor == "win":
        master_ninja.rule(
            ...
            command="%s nnabt-win-tool recursive-mirror $in $out" % sys.executable,
        )
    elif flavor == "zos":
        master_ninja.rule(
            "copy",
    ...
    for build_file in params["build_files"]:
        for target in nnabt.common.AllTargets(
    ...
    for qualified_target in target_list:
        # qualified_target is like: third_party/icu/icu.nnabt:icui18n#target
        build_file, name, toolset = nnabt.common.ParseQualifiedTarget(qualified_target)
        ...
        build_file = nnabt.common.RelativePath(build_file, options.toplevel_dir, False)

        qualified_target_for_hash = nnabt.common.QualifiedTarget(
    ...
    if target_short_names:
        ...
        # "build chrome" case as well as the nnabt tests, which expect to be
...
def GenerateOutput(target_list, target_dicts, data, params):
    ...
    target_dicts = nnabt.xcode_emulation.CloneConfigurationForDeviceAndEmulator(
        target_dicts
    )

    user_config = params.get("generator_flags", {}).get("config", None)
    if nnabt.common.GetFlavor(params) == "win":
