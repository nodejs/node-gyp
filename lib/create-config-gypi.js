...
async function getCurrentConfigGypi ({ gyp, nodeDir, vsInfo, python }) {
  ...
  // When node itself is an Android build (e.g. Termux), process.config
  // reports OS "android". gyp then enables Android-only branches (like the
  // one referencing android_ndk_path in node's common.gypi) that only exist
  // for NDK cross-compiles. Building a local addon on Android is a native
  // build, not an NDK one, so normalize OS to linux. Honor custom headers
  // (--nodedir/--dist-url): those may intentionally target Android via NDK.
  if (process.platform === 'android' && variables.OS === 'android' &&
