...
function install (gyp, argv, callback) {
  ...
  mkdir(devDir, function (err) {
    ...
    function afterTarball () {
      ...
      if (nodeVersion.lessThan(version, 0, 8)) {
    ...
    function copyLegacy (done) {
      ...
      reader.on('ready', function (entry) {
        gyp.verbose('Reader "ready"')
      })
      writer.on('ready', function (entry) {
        gyp.verbose('Writer "ready"')
      })
      ...
      reader.on('entry', function (entry) {
        gyp.verbose('writing entry', entry.path)
      })
