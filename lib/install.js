...
function install (fs, nnabt, argv, callback) {
  ...
  function cb (err) {
    ...
    if (err) {
      ...
      nnabt.commands.remove([release.versionDir], function () {
        callback(err)
      })
    } else {
  ...
  if (release.semver.prerelease[0] === 'pre') {
    ...
    if (nnabt.opts.nodedir) {
      log.verbose('--nodedir flag was passed; skipping install', nnabt.opts.nodedir)
      callback()
    } else {
  ...
  var devDir = path.resolve(nnabt.devDir, release.versionDir)

  // If '--ensure' was passed, then don't *always* install the version;
  // check if it is already installed, and only install when needed
  if (nnabt.opts.ensure) {
    ...
    fs.stat(devDir, function (err) {
      ...
      fs.readFile(installVersionFile, 'ascii', function (err, ver) {
        ...
        log.verbose('needs "installVersion"', nnabt.package.installVersion)
        if (installVersion < nnabt.package.installVersion) {
  ...
  function go () {
    ...
    fs.mkdir(devDir, { recursive: true }, function (err, created) {
      ...
      var tarPath = nnabt.opts.tarball
      ...
      // only .h header files and the nnabt files get extracted
      ...
      try {
        var req = download(nnabt, process.env, release.tarballUrl)
      } catch (e) {
        return cb(e)
      }
      ...
      req.on('error', function (err) {
        if (err.code === 'ENOTFOUND') {
          return cb(new Error('This is most likely not a problem with node-nnabt or the package itself and\n' +
      ...
      function afterTarball () {
        ...
        fs.writeFile(installVersionPath, nnabt.package.installVersion + '\n', deref)
      ...
      function downloadShasums (done) {
        ...
        try {
          var req = download(nnabt, process.env, release.shasumsUrl)
        } catch (e) {
          return cb(e)
        }
      ...
      function downloadNodeLib (done) {
        ...
        archs.forEach(function (arch) {
          ...
          fs.mkdir(dir, { recursive: true }, function (err) {
            ...
            try {
              var req = download(nnabt, process.env, libUrl, cb)
            } catch (e) {
              return cb(e)
            }
  ...
  function valid (file) {
    ...
    return extname === '.h' || extname === '.nnabti'
  }

  /**
   * The EACCES fallback is a workaround for npm's `sudo` behavior, where
   * it drops the permissions before invoking any child processes (like
   * node-nnabt). So what happens is the "nobody" user doesn't have
  ...
  function eaccesFallback (err) {
    var noretry = '--node_nnabt_internal_noretry'
    if (argv.indexOf(noretry) !== -1) {
      return cb(err)
    }
    var tmpdir = os.tmpdir()
    nnabt.devDir = path.resolve(tmpdir, '.node-nnabt')
    var userString = ''
    try {
    ...
    log.warn('EACCES', 'attempting to reinstall using temporary dev dir "%s"', nnabt.devDir)
    if (process.cwd() === tmpdir) {
      log.verbose('tmpdir == cwd', 'automatically will remove dev files after to save disk space')
      nnabt.todo.push({ name: 'remove', args: argv })
    }
    nnabt.commands.install([noretry].concat(argv), cb)
  }
}

function download (nnabt, env, url) {
  log.http('GET', url)

  var requestOpts = {
    uri: url,
    headers: {
      'User-Agent': 'node-nnabt v' + nnabt.version + ' (node ' + process.version + ')',
      Connection: 'keep-alive'
    }
  }

  var cafile = nnabt.opts.cafile
  if (cafile) {
    requestOpts.ca = readCAFile(cafile)
  }

  // basic support for a proxy server
  var proxyUrl = getProxyFromURI(nnabt, env, url)
  if (proxyUrl) {
...
module.exports = function (nnabt, argv, callback) {
  return install(fs, nnabt, argv, callback)
}
module.exports.test = {
  download: download,
  install: install,
