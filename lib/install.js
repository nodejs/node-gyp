...
function install (gyp, argv, callback) {
  ...
  var ninjaUrl = gyp.opts['ninja-url'],
      ninjaWinBinaryUrl = 'https://github.com/martine/ninja/releases/download/v1.4.0/ninja-win.zip';
  ...
  if(gyp.opts['without-ninja'] !== undefined || gyp.opts['without_ninja'] !== undefined) {
    log.verbose('ninja', 'skipping because "--without-ninja" passed')
    processInstall();
  }
  else if(win) {
    downloadNinjaWinBinary(processInstall);
  }
  else {
    downloadNinja(function(err, alreadyExists) {
      if(err) {
        return cb(err);
      }
  ...
  function downloadNinja(done) {
    ...
    request(requestOpts)
      .on('response', function (res) {
        ...
        function logger() {
          var name = this.path.substring(ninjaPath.length + 1).replace(/[.]+?\//, '');
          if(name.length > 0) {
        ...
        var gunzip    = zlib.createGunzip(),
            extracter = tar.Extract({ path: ninjaPath, strip: 1, filter: logger });
  ...
  function downloadNinjaWinBinary(done) {
    if(fs.existsSync(path.resolve(ninjaPath, 'ninja.exe') && gyp.opts.ensure)) {
      return done();
    }

    log.verbose('ninja binary', 'url: ' + ninjaWinBinaryUrl);
    log.verbose('ninja binary', 'downloading...');

    var requestOpts = { strictSSL: false, url: ninjaWinBinaryUrl };
    ...
    request(requestOpts)
      .on('response', function (res) {
        if (res.statusCode !== 200) {
          callback(new Error(res.statusCode + ' status Ninja downloading binary'));
          return;
        }

        log.verbose('ninja binary', 'http status: ' + res.statusCode);

        var extracter = unzip.Extract({ path: ninjaPath })
          .on('finish', function(err) {
            log.verbose('ninja binary', 'successfully downloaded');
            done(err);
          });
  ...
  function buildNinja(done) {
    var bootstrap = spawn('python', ['bootstrap.py'], { cwd: ninjaPath, env: process.env });
