...
function build (gyp, argv, callback) {
  ...
  function loadConfigGypi () {
    fs.readFile(configPath, 'utf8', function (err, data) {
      ...
      defineBuildCommand(config.variables.generators);
  ...
  function defineBuildCommand(generators) {
    if(~generators.indexOf('ninja')) {
    ...
    if(!command) {
      makeCommand = ~process.platform.indexOf('bsd') && !~process.platform.indexOf('kfreebsd') ? 'gmake' : 'make';
    }

    if(~generators.indexOf('make')) {
      command = makeCommand;
      generator = 'make';
      return;
    }

    if(~generators.indexOf('msvs')) {
      command = 'msbuild';
      generator = 'msvs';
      return;
    }
  ...
  function doBuild () {
    ...
    if (generator === 'make' && verbose) {
      argv.push('V=1')
    }
    if (generator === 'msvs' && !verbose) {
