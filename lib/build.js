...
function build (gyp, argv, callback) {
  ...
  var command = win ? 'msbuild' : 'make'
    , afterWhich = win ? findSolutionFile : build
    , emitter
  ...
  /**
   * Find first build/*.sln file.
   */

  function findSolutionFile (done) {
    glob('build/*.sln', function (err, files) {
      if (err) return done(err)
      if (files.length === 0) {
        return done(new Error('Could not find *.sln file. Did you run "configure"?'))
      }
