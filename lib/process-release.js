...
function processRelease (argv, nnabt, defaultVersion, defaultRelease) {
  var version = (semver.valid(argv[0]) && argv[0]) || nnabt.opts.target || defaultVersion
  var versionSemver = semver.parse(version)
  var overrideDistUrl = nnabt.opts['dist-url'] || nnabt.opts.disturl
  ...
  if (defaultRelease) {
    ...
    // published release assets as "iojs" so that the node-nnabt logic worked.  Once Electron@3 has
