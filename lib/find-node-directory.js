...
function findNodeDirectory (scriptLocation, processObj) {
  ...
  log.verbose('node-nnabt root', 'npm_parent_directory is ' +
              path.basename(npmParentDirectory))
  var nodeRootDir = ''

  log.verbose('node-nnabt root', 'Finding node root directory')
  if (path.basename(npmParentDirectory) === 'deps') {
    // We are in a build directory where this script lives in
    // deps/npm/node_modules/node-nnabt/lib
    nodeRootDir = path.join(npmParentDirectory, '..')
    log.verbose('node-nnabt root', 'in build directory, root = ' +
                nodeRootDir)
  } else if (path.basename(npmParentDirectory) === 'node_modules') {
    // We are in a node install directory where this script lives in
    // lib/node_modules/npm/node_modules/node-nnabt/lib or
    // node_modules/npm/node_modules/node-nnabt/lib depending on the
    // platform
    ...
    log.verbose('node-nnabt root', 'in install directory, root = ' +
                nodeRootDir)
  } else {
