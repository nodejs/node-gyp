...
function configure (gyp, argv, callback) {
  ...
  function createConfigFile (err, vsSetup) {
    ...
    if (vsSetup) {
      // GYP doesn't (yet) have support for VS2017/VS2019, so we force it to VS2015
      ...
      try {
        // Visual Studio 2019
        msbuildPath = path.join(vsSetup.path, 'MSBuild', 'Current', 'Bin', 'MSBuild.exe')
        if (!fs.existsSync(msbuildPath)) {
          // Visual Studio 2017
          msbuildPath = path.join(vsSetup.path, 'MSBuild', '15.0', 'Bin', 'MSBuild.exe')
          if (!fs.existsSync(msbuildPath)) {
            var msg = msgFormat('Could not find msbuild.exe in %s', vsSetup.path)
            log.error('Could not find msbuild.exe')
      ...
      variables['msbuild_path'] = msbuildPath
    }
