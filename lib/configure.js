...
async function configure (gyp, argv) {
  ...
  async function runGyp () {
    ...
    await new Promise((resolve, reject) => {
      // Capture gyp's stderr (still forwarded to the terminal) so we can
      // recognise known failure modes and print actionable hints.
      const cp = gyp.spawn(python, argv, { stdio: ['inherit', 'inherit', 'pipe'] })
      let gypStderr = ''
      if (cp.stderr) {
      ...
      cp.on('exit', (code) => {
        if (code !== 0) {
          // Only hint when the undefined variable comes from the Node/NDK
          // header toolchain (android_ndk_* variables, or any variable while
          // loading common.gypi), not when a binding.gyp itself references an
          // undefined variable — that is a project bug and this hint would
          // send the user down the wrong path.
          const ndkHint = process.platform === 'android' &&
            isNdkUndefinedVariableHint(gypStderr)
          if (ndkHint) {
            log.error('gyp', [
              'The Node headers downloaded for this target reference NDK',
              'variables, but no NDK is installed. On Android (e.g. Termux)',
              'you usually want to compile against the system toolchain instead:',
              'make sure node-gyp uses the locally installed headers',
              '($PREFIX/include/node) rather than the official ones downloaded',
              'from nodejs.org, e.g. by rebuilding Node with',
              '--use-prefix-to-find-headers or by passing --nodedir pointing at',
              'your local headers.'
            ].join('\n'))
          }
...
/**
 * Detect the "Undefined variable" gyp failure that comes from the Node/NDK
 * header toolchain (android_ndk_* variables, or any undefined variable while
 * loading common.gypi) rather than from the project's own binding.gyp.
 * Exported for tests.
 * @param {string} gypStderr - captured gyp stderr (also forwarded to the
 * terminal byte-for-byte by the caller).
 * @returns {boolean}
 */
function isNdkUndefinedVariableHint (gypStderr) {
  return /Undefined variable android_ndk_/.test(gypStderr) ||
    /Undefined variable [A-Za-z0-9_]+ in .*common\.gypi/.test(gypStderr)
}

module.exports = configure
module.exports.isNdkUndefinedVariableHint = isNdkUndefinedVariableHint
