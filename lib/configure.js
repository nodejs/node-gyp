...
const usage = `Generates ${win ? 'MSVC project files' : 'a Makefile'} for the current module`
...
function configure (gyp, argv, callback) {
  ...
  findPython(gyp.opts.python, (err, found) => {
    if (err) {
      callback(err)
    } else {
      python = found
  ...
  function getNodeDir () {
    ...
    if (gyp.opts.nodedir) {
      ...
      gyp.commands.install([release.version], (err) => {
        if (err) {
          return callback(err)
        }
  ...
  function createConfigFile (err, vsInfo) {
    ...
    const boolsToString = (k, v) => typeof v === 'boolean' ? String(v) : v
  ...
  function runGyp (err) {
    ...
    configs.forEach((config) => argv.push('-I', config))
...
function findAccessibleSync (logprefix, dir, candidates) {
  for (const candidate of candidates) {
    ...
    try {
      ...
      log.silly(logprefix, 'Could not open %s: %s', candidatePath, e.message)
      continue
    }
    fs.closeSync(fd)
    log.silly(logprefix, 'Found readable %s', candidatePath)
