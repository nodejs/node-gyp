...
function configure (gyp, argv, callback) {
  ...
  function runGyp (err) {
    ...
    var config = fs.readFileSync(configPath, { encoding: 'UTF-8' }).replace(/^#.+$/gm, '');
    // parse json
    var variables = JSON.parse(config).variables;

    variables.generators.forEach(function(generator) {
