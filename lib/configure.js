...
function configure (gyp, argv, callback) {
  ...
  function createConfigFile (err) {
    ...
    if (!gyp.opts.nodedir) {
      variables.nodedir = path.resolve(gyp.devDir, version)
    } else {      
      // simple ~ homedir expansion based on https://github.com/joyent/node/issues/2857
      if (win) {
        gyp.opts.nodedir = gyp.opts.nodedir.replace(/^~/, process.env['HOMEDRIVE'] + process.env['HOMEPATH'])
      } else {
        gyp.opts.nodedir = gyp.opts.nodedir.replace(/^~/, process.env['HOME'])
      }      

      variables.nodedir = gyp.opts.nodedir
    }
