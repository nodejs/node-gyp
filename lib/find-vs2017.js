...
function findVS2017() {
  ...
  var ps = 'PowerShell -ExecutionPolicy Unrestricted -Command '
  var csFile = path.join(__dirname, 'Find-VS2017.cs')
  var psQuery = ps + '"&{Add-Type -Path \'' + csFile + '\'; [VisualStudioConfiguration.Main]::Query()}" 2>&1'

  var vsSetup
  try {
    var vsSetupRaw = execSync(psQuery, { encoding: 'utf8' })
